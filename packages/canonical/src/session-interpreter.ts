...
import type {
  AtomEdge, CapturedBlob, ConversationAtom, DerivedCandidate, RawRecord, SourceFragment,
  SessionProjection, UserTurnProjection, TurnContextProjection, ToolCallProjection,
  DisplaySegment, UserMessageProjection, ParsedSessionMetadata, GitProjectEvidence,
  TokenUsageMetrics, ParsedSessionEvidence, SessionInterpretation,
} from "@cchistory/domain";
import {
  joinDisplaySegments, applyMaskTemplates, LITERAL_PROMPT_MASK_TEMPLATE_IDS,
  stableId, asString, nowIso, truncate, isObject, mergeTokenUsageMetrics, sha1,
  normalizeGitRemote, normalizeLocalPathIdentity as normalizeWorkspacePath,
  findLastConversationActivityAtom, isUserTurnAtom, extractTokenUsageFromPayload,
  extractTokenCountFromPayload, extractStopReasonFromPayload, EVIDENCE_RULE_VERSION as RULE_VERSION,
} from "@cchistory/domain";

const normalizePathSeparators = (value: string): string => value.replace(/\\/g, "/");

/** Interpret reconciled evidence once. This entry currently constructs complete detail. */
export function interpretSessionEvidence(evidence: ParsedSessionEvidence): SessionInterpretation {
  const { draft, atoms, fragments, records, blobs } = evidence;
  const projectCandidates = buildProjectObservationCandidates(draft, atoms, evidence.gitProjectEvidence);
  const submission = buildSubmissionGroups(draft, atoms, evidence.edges);
  const result = buildTurnsAndContext(draft, fragments, records, blobs, atoms, submission.groups, submission.edges);
  const suppressed = draft.source_platform === "codebuddy" && result.turns.length === 0 && atoms.length === 0;
  return {
    session: suppressed ? undefined : result.session,
    candidates: suppressed ? [] : [...projectCandidates, ...submission.groups, ...result.turnCandidates, ...result.contextCandidates],
    turns: suppressed ? [] : result.turns,
    contexts: suppressed ? [] : result.contexts,
    // Preserve the legacy payload's edge multiplicity during this ownership move.
    // The old builder appended submission edges in place, then flattened that array twice.
    edges: [...submission.edges, ...submission.edges],
  };
}

export function buildProjectObservationCandidates(
  draft: ParsedSessionMetadata,
...
export function buildSubmissionGroups(draft: ParsedSessionMetadata, atoms: readonly ConversationAtom[], inputEdges: readonly AtomEdge[]): {
...
export function buildTurnsAndContext(
  ...
  atoms: readonly ConversationAtom[],
  submissionGroups: readonly DerivedCandidate[],
...
export function buildTurnContext(
  turnId: string,
  draft: ParsedSessionMetadata,
  groupAtoms: readonly ConversationAtom[],
  contextAtoms: readonly ConversationAtom[],
